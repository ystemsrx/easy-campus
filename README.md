...
- 教务通知实时搜索、分页和正文详情；章节、编号列表、项目符号、表格、链接与文字强调均按服务端安全语义结果展示。
- 成绩查询、真实学年与学期筛选、搜索、排序、分页、主动刷新、教师、成绩性质及可靠的成绩分项；补考和缓考只展示正式总评，第三学期仅在用户确有记录时显示。
- 数字、等级制、`缓考`、`违纪` 等任意文字成绩均按接口原值展示。
- 空教室按日期、节次、校区和最多 30 栋楼查询，所有编码均从后端实时读取。
- 考试只按教务系统返回的本人有效学期查询，默认最新学期；重修直接使用教务重修标记，补缓考只根据考试名称标记。
...
Skyline 页面不内嵌教务系统 `web-view`，也不会把后端持有的校园会话交给系统浏览器。进入通知详情时，小程序请求后端解析后的安全 `contentHtml` 并使用 `rich-text` 展示；服务端会把教务页面中常见的 Word 段落式编号重建为易读的缩进列表。原始教务链接仅作为底部辅助来源供复制，不再代替正文。
