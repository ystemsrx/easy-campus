...
体验版（`trial`）和正式版（`release`）统一使用 `https://easy-api.lazycampus.com`。该 HTTPS 域名也必须在微信公众平台配置为 `request` 与 `downloadFile` 合法域名。
...
- 成绩和课表优先使用账号隔离的本地与服务端快照；距上次成功刷新满 15 天才自动更新，下拉刷新会传入 `refresh=true` 强制更新。
- 教务消息和通知只在服务端与本地各持久化最新十五条预览；页面先展示旧预览，自动访问教务系统的间隔不少于 24 小时，主动刷新不受该间隔限制。
- 退出登录只撤销会话，服务器资料、成绩和十五条消息/通知预览不会删除；下次输入相同密码可快速进入。
