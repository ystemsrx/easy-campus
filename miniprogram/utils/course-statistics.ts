...
export function normalizeCourseName(value: string): string {
  return value.normalize("NFKC").replace(/\s+/g, " ").trim().slice(0, 255);
}

export function courseStatisticsKey(value: string): string {
  const name = normalizeCourseName(value).toLocaleLowerCase("zh-CN");
