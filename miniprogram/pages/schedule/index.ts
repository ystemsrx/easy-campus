...
}from"../../store/preferences";import{captureSessionLease as takeLease,getSession,isSessionLeaseCurrent as leaseCurrent,type SessionLease as Lease,
...
}from"../../types/api";import{resolveAppearance}from"../../utils/appearance";import{toDateString as dateKey}from"../../utils/date";import{haptic}from"../../utils/haptics";import{ensureAuthenticated,navigateTo}from"../../utils/navigation";type Touch=WechatMiniprogram.TouchEvent;type Event<T extends WechatMiniprogram.IAnyObject>=WechatMiniprogram.CustomEvent<T>;let table:Table|null=null;const DAY_LABELS=["一","二","三","四","五","六","日"];const ROW_H=92;const WEEKDAY_H=32;type CalendarDay=Day&{hasCourse:boolean;outside:boolean};function calendarMarks(date:string,timetable:Table|null,plans:Plan[]){return{hasPlan:plans.some((plan)=>planOccursOnDate(plan,date)),hasCourse:coursesForDate(timetable,date,asDate(date)).length>0,
};
}
function makeGrid(date:string,selectedDate:string,timetable:Table|null,plans:Plan[]){const selected=asDate(date);const first=new Date(selected.getFullYear(),selected.getMonth(),1,12);const firstWeekday=wd(first)-1;const count=6;const firstDate=shiftScheduleDate(dateKey(first),-firstWeekday);const today=dateKey(new Date());const rows=Array.from({length:count},(_,row)=>({row,days:Array.from({length:7},(_,column):CalendarDay=>{const key=shiftScheduleDate(firstDate,row*7+column);const day=asDate(key);return{weekday:(column+1)as Day["weekday"],shortLabel:DAY_LABELS[column],dateLabel:String(day.getDate()),date:key,isToday:key===today,outside:day.getMonth()!==selected.getMonth(),...calendarMarks(key,timetable,plans),
};
}),
}));return{monthRows:rows,monthAnchor:date,monthSelected:date.slice(0,7)===selectedDate.slice(0,7),monthHeight:WEEKDAY_H+count*ROW_H,
monthTitle:`${selected.getMonth()+1}月`,
yearTitle:String(selected.getFullYear()),
};
}
function makeMonth(date:string,selectedDate:string,timetable:Table|null,plans:Plan[]){const center=makeGrid(date,selectedDate,timetable,plans),month=asDate(date),picked=asDate(selectedDate);const monthPanels=[-1,0,1].map((offset,slot)=>({...(offset?makeGrid(dateKey(new Date(month.getFullYear(),month.getMonth()+offset,1,12)),selectedDate,timetable,plans):center),slot}));return{...center,monthPanels,monthOffset:(picked.getFullYear()-month.getFullYear())*12+picked.getMonth()-month.getMonth()};}
...
let acct = "";let tl: Lease | null = null;let syncLease: Lease | null = null;let warmRev = 0;let tbStamp = 0;let schedRev: string | null = null;let hydrated: SSR | null = null;let rt: ReturnType<typeof setTimeout> | undefined;let shown = false;let moving = false;let dirty = false;let pd = "";let savedPlan: { date: string; id: string } | null = null;let scrollY = new Map<string, number>();let swipeEnd = 0;let foldEnd = 0;type SV = WechatMiniprogram.Skyline.SharedValue<number>;interface ScheduleMotion {position: SV;start: SV;width: SV;weekWidth: SV;grid:SV;shade:SV;from:SV;to:SV;ready: SV;active: SV;sequence: SV;
}
...
function scheduleSourcesAreCurrent(account:string):boolean {if (!hydrated || hydrated.account !== account) {return false;
}
const current=sourceRevs(account);return (
current.date === hydrated.date &&
SOURCE_NAMES.every(
(source) => current[source] === hydrated?.[source],)
);
}
function markSources(
account:string,sources:readonly SSN[],):void {if (!hydrated || hydrated.account !== account) {return;
}
const current=sourceRevs(account);const next={ ...hydrated,date:current.date };for (const source of sources) next[source]=current[source];hydrated=next;
}
...
WechatMiniprogram.Skyline.SharedValue<number[]> | undefined,_motion: null as ScheduleMotion | null,_viewReady: false,_headerRendered: false,_headerBindingsStarted: false,_settling:false,_touchMonth: null as { x: number; y: number } | null,_foldTouchStart: null as { y: number; progress: number; moved: boolean } | null,_nativeCurrent: 7 + wd() - 1,data: {homeRemoved: false,...INIT_STYLE,currentTime: "",monthLabel: "",teachingWeekLabel: "",days: [] as Day[],selectedWeekday: wd(),selectedDate: dateKey(new Date()),selectedDateLabel: "",entries: [] as ScheduleEntry[],dayPages: [] as ReturnType<typeof makePager>["dayPages"],weekPages: [] as ReturnType<typeof makePager>["weekPages"],dayCurrent: 7 + wd() - 1,dayAnimated: false,daySlide:"",headerMotionReady: false,monthOpen: false,foldProgress: 0,foldDragging: false,mx:0,swiping:false,monthAnchor: dateKey(new Date()),monthOffset:0,monthPanels:[] as ReturnType<typeof makeMonth>["monthPanels"],weekdays: DAY_LABELS,monthHeight: 128,monthTitle: "",yearTitle: "",dayScrollTops: [] as number[],focusedPlanId: "",timelineHeight: SCHEDULE_TIMELINE_HEIGHT,creating: false,creatorMode: "plan" as "plan" | "course",creatorSheetHeight: 58,courseTimeOpen: false,courseWeeksOpen: false,courseTimeExpandHeight: 0,courseWeeksExpandHeight: 0,courseLocation: "",courseTeacher: "",savingCourse: false,editingCourseId: "",editingCourseDate: "",courseWeekdays: [wd()] as number[],courseWeekdayOptions: DAY_LABELS.map((label, index) => ({ weekday: index + 1, label, selected: index + 1 === wd() })),coursePeriods: [] as number[],courseWeeks: [] as number[],coursePeriodOptions: [] as Array<{period: number;time: string;selected: boolean;
...
},onLoad() {enableTimelineShare();initializeCapsuleBackdrop(this);moving = false;dirty = false;pd = "";savedPlan = null;scrollY = new Map();const position = dayIx(this.data.selectedDate);if (wx.worklet?.shared)
...
(wx.getWindowInfo().windowWidth * 670) / 750,),grid:wx.worklet.shared(position),shade:wx.worklet.shared(1),from:wx.worklet.shared(position),to:wx.worklet.shared(position),ready:wx.worklet.shared(0),active:wx.worklet.shared(0),sequence:wx.worklet.shared(0),
};shown = false;hydrated = null;const account = getSession()?.user.account || "";if (account) this.hydrateCachedScheduleIfNeeded(account, true);
...
"-" +
weekday +
(selected ? "-selected" : "-normal"),() => {"worklet";const position = dp.value;const week = Math.floor(position / 7);const offset = (slot - (week % 3) + 3) % 3;const slotWeek = week + (offset === 2 ? -1 : offset);const day = slotWeek * 7 + weekday - 1;const weight = Math.max(0, 1 - Math.abs(position - day));return { opacity: "" + (selected ? weight : 1 - weight) };
},config,bound,);
}
}
}
...
host.applyAnimatedStyle(".orb-caption",()=>{"worklet";return{opacity:""+(motion.active.value?0:motion.shade.value*Math.max(0,1-Math.abs(dp.value-to.value)*4))}},config,bound);
for(let row=0;row<6;row++)for(let column=1;column<=7;column++)for(const selected of [false,true])host.applyAnimatedStyle(".md-"+row+"-"+column+(selected?"-selected":"-normal"),()=>{"worklet";const day=grid.value+row*7+column-1,weight=Math.max(0,1-Math.abs(dp.value-day))*motion.shade.value;return{opacity:""+(selected?weight:1-weight)}},config,bound);
...
},onShow() {this.setData({ homeRemoved: navigationIndex("home") < 0 });if (!ensureAuthenticated()) return;shown = true;attachCapsuleBackdrop(this, "schedule");const account = getSession()?.user.account || "";if (!account) return;this.hydrateCachedScheduleIfNeeded(account);const tabBar = this.getTabBar();if (tabBar) {tabBar.setData({selected: navigationIndex("schedule"),themeClass: this.data.themeClass,visualThemeClass: this.data.visualThemeClass,motionClass: this.data.motionClass,hidden: false,
});
}
this.scheduleBackgroundRefresh(REFRESH_DELAY);
},onHide() {detachCapsuleBackdrop(this);shown = false;clearRefresh();swipeEnd = 0;foldEnd = 0;moving = false;pd = "";if (this._motion) {const day = this.data.dayPages[this._nativeCurrent];if (day) this.setData({ selectedDate: day.selectedDate });this._motion.active.value = 0;
}
...
},hydrateCachedScheduleIfNeeded(account:string,force=false):boolean {if (!force && scheduleSourcesAreCurrent(account)) return false;const previous=hydrated;const current=sourceRevs(account);const accountChanged=!previous || previous.account !== account;const preferencesChanged =
...
? candidate
:null;acct=account;if (warm) {table=addCourses(
...
patch,makeDay(
table,schedule.plans,accountChanged ? dateKey(new Date()) :this.data.selectedDate,),);
}
if (accountChanged) {scrollY.clear();moving = false;pd = "";savedPlan = null;Object.assign(patch, {creating: false,editingPlanId: "",title: "",focusedPlanId: "",dayScrollTops: [],
});
}
const date=String(patch.selectedDate || this.data.selectedDate);Object.assign(
patch,warm?.pager ||
makePager(table,schedule.plans,date),);
}
hydrated=sourceRevs(account);if (contentChanged) this.replaceDayWindow(patch);else if (Object.keys(patch).length) this.setData(patch);return true;
...
},async loadTimetable() {const lease=takeLease();if (!lease) return;if (tl && leaseCurrent(tl)) {return;
}
...
this.applyPrewarmedSchedule();const current=loadTimetableSnapshot(lease.account);const storedAt=current?.localStoredAt || 0;if (storedAt !== tbStamp) {table=addCourses(
...
} finally {if (tl === lease) tl=null;if (shouldRefreshAfterward && leaseCurrent(lease)) {setTimeout(() => {if (
shown &&
leaseCurrent(lease) &&
acct === lease.account
...
},async refreshTimetable() {const lease=takeLease();if (!lease || acct !== lease.account) return;if (tl && leaseCurrent(tl)) {return;
}
tl=lease;try {const result=await getTimetable({ refresh:true,automatic:true });if (!leaseCurrent(lease) || acct !== lease.account) {return;
}
const local=loadTimetableSnapshot(lease.account);if (!shouldStoreServerSnapshot(local,result.meta,true)) return;table=addCourses(
...
},async syncSchedule() {const lease=takeLease();if (!lease || acct !== lease.account) return;if (syncLease && leaseCurrent(syncLease)) return;syncLease=lease;try {await getPreloadedSchedule();if (
!shown ||
!leaseCurrent(lease) ||
acct !== lease.account
) {return;
...
} finally {if (syncLease === lease) syncLease=null;
}
...
},replaceDayWindow(patch:Record<string,unknown>,direction=0) {const motion=this._motion,old=motion?.position.value;this.decorateCalendarPatch(patch);if (direction && this.data.motionClass !== "motion-reduced") patch.daySlide=direction > 0 ? "next" :"previous";if (motion) {motion.ready.value=0;motion.sequence.value += 1;
}
const sequence=motion?.sequence.value;moving=true;this.setData({ dayAnimated:false },() => {if (this._motion !== motion || motion?.sequence.value !== sequence)
...
return;if (patch.daySlide) wx.nextTick(() => {if (motion?.sequence.value === sequence) this.setData({daySlide:""})});if (motion) motion.ready.value=1;moving=false;this.flushPendingDate();
});
});
});
},decorateCalendarPatch(patch:Record<string,unknown>) {const date=String(patch.selectedDate || this.data.selectedDate);const plans=sch(acct).plans;const anchor=String(patch.monthAnchor || (date === this.data.selectedDate ? this.data.monthAnchor :date));Object.assign(patch,makeMonth(anchor,date,table,plans));if (patch.weekPages) {patch.weekPages=markWeekPages(
patch.weekPages as ReturnType<typeof makePager>["weekPages"],table,plans,);
}
},rebuildWeek(forceRebase=false) {if (moving || this._motion?.active.value) {dirty=true;return;
}
...
table,schedule.plans,date,windowStart,);const patch={...makeDay(table,schedule.plans,date),...pager,dayScrollTops:pager.dayPages.map(
(day) => scrollY.get(day.selectedDate) || 0,),
};dirty=false;if (
forceRebase ||
...
} else {this.decorateCalendarPatch(patch);this.setData(patch);
}
markSources(acct, ["timetable", "schedule"]);
},onDayScrollStart() {"worklet";const motion = this._motion;if (!motion || !motion.ready.value) return;motion.from.value=motion.start.value;motion.to.value=motion.start.value;motion.active.value = 1;motion.sequence.value += 1;const markDayScrolling = this.markDayScrolling.bind(this);if (runOnJS) runOnJS(markDayScrolling)(motion.sequence.value);
...
},finishDayScroll(position:number,sequence:number) {const motion=this._motion;if (
!motion ||
!motion.ready.value ||
...
moving=false;const anchor=this.data.monthAnchor,returnMonth=this.data.monthOpen&&anchor.slice(0,7)!==day.selectedDate.slice(0,7),calendarPatch=makeMonth(returnMonth?anchor:day.selectedDate,day.selectedDate,table,sch(acct).plans);this.setData({dayCurrent:current,selectedDate:day.selectedDate,days:day.days,selectedWeekday:day.selectedWeekday,selectedDateLabel:day.selectedDateLabel,teachingWeekLabel:day.teachingWeekLabel,monthLabel:day.monthLabel,entries:day.entries,...calendarPatch,
},() => {this.syncOrb();motion.to.value=dayIndex;if(returnMonth)this.changeCalendarMonth(dayIx(day.selectedDate)>dayIx(anchor)?1:-1,day.selectedDate)});if (weekChanged || current === 0 || current === 20 || dirty)
this.rebuildWeek();this.flushPendingDate();
},flushPendingDate() {if (moving || this._motion?.active.value) return;const next = pd;pd = "";if (next && next !== this.data.selectedDate)
this.navigateScheduleDate(next);else if (savedPlan?.date === this.data.selectedDate) {this.setData({ focusedPlanId:savedPlan.id });savedPlan=null;
}
},onGlassDayScroll(
...
event: Event<{ scrollTop: number }>,) {const date = String(event.currentTarget.dataset.date || "");if (date) scrollY.set(date, event.detail.scrollTop);const slot = Number(event.currentTarget.dataset.slot);if (this._capsuleDayScroll && slot >= 0) {const values = this._capsuleDayScroll.value.slice();values[slot] = event.detail.scrollTop;this._capsuleDayScroll.value = values;
}
if (scrollY.size > 90)
scrollY.delete(scrollY.keys().next().value!);
},navigateScheduleDate(date:string) {if (moving || this._motion?.active.value) {pd=date;return;
}
...
) {const plans=sch(acct).plans;const pager=makePager(table,plans,date);this.replaceDayWindow({...makeDay(table,plans,date),...pager,dayScrollTops:pager.dayPages.map(
(day) => scrollY.get(day.selectedDate) || 0,),
},this.data.monthOpen ? dayIx(date)-dayIx(this.data.selectedDate) :0);return;
}
moving=true;this.setData({ dayCurrent:current });
},selectDay(event: Touch) {if (Date.now() < swipeEnd) return;savedPlan = null;const date = String(event.currentTarget.dataset.date || "");if (date) this.navigateScheduleDate(date);
...
},changeCalendarMonth(direction:number,target?:string) {if (!this.data.monthOpen||this._settling) return;this._settling=true;const selected=asDate(this.data.monthAnchor),picked=target?asDate(target):new Date(selected.getFullYear(),selected.getMonth()+direction,1,12),key=dateKey(new Date(picked.getFullYear(),picked.getMonth(),1,12)),width=this._motion?.weekWidth.value||wx.getWindowInfo().windowWidth*670/750,slot=direction>0?2:0;const slide=()=>this.setData({swiping:false,mx:this.data.motionClass==="motion-reduced"?0:-direction*width},()=>setTimeout(()=>{this.setData({...makeMonth(key,this.data.selectedDate,table,sch(acct).plans),swiping:true,mx:0},()=>{this.syncOrb();this._settling=false;this.setData({swiping:false})})},this.data.motionClass==="motion-reduced"?0:260));if(target&&this.data.monthPanels[slot]?.monthAnchor.slice(0,7)!==key.slice(0,7))this.setData({[`monthPanels[${slot}]`]:{...makeGrid(key,this.data.selectedDate,table,sch(acct).plans),slot},monthOffset:direction},slide);else slide();
...
},previousCalendarMonth() { this.changeCalendarMonth(-1); },nextCalendarMonth() { this.changeCalendarMonth(1); },monthStart(event:Touch) {const touch=event.touches[0];if (touch&&!this._settling) this._touchMonth={ x:touch.clientX,y:touch.clientY };
},monthMove(event:Touch) {const start=this._touchMonth,touch=event.touches[0];if (!start || !touch) return;const dx=touch.clientX-start.x,dy=touch.clientY-start.y;if (Math.abs(dx) < 5 || Math.abs(dx) < Math.abs(dy)*1.25) return;this.setData({swiping:true,mx:dx});
},monthEnd(event:Touch) {const start=this._touchMonth;this._touchMonth=null;if (!start) return;const touch=event.changedTouches[0]||{clientX:start.x,clientY:start.y};const dx=touch.clientX-start.x,dy=touch.clientY-start.y;if (Math.abs(dx) < 36 || Math.abs(dx) < Math.abs(dy)*1.25) {this.setData({swiping:false,mx:0});return}swipeEnd=Date.now()+320;this.changeCalendarMonth(dx < 0 ? 1 :-1);
},goToday() {savedPlan = null;haptic("light");this.navigateScheduleDate(dateKey(new Date()));
...
? nextStart.startDate
: this.data.selectedDate;const startTime = nextStart.startTime;const defaultEnd = defaultPlanEnd(startDate, startTime);this.setTabBarHidden(true);const draft = loadInteractionDraft(acct, "schedule");const periods = table?.periods || [];const defaultWeek = table
? timetableWeekForDisplay(table)
: 1;const weekCount = table ? timetableWeekCount(table) : 0;this.setData({creating: true,creatorMode: "plan",creatorSheetHeight: 58,courseTimeOpen: false,courseWeeksOpen: false,courseTimeExpandHeight: 112 + Math.ceil(periods.length / 3) * 112,courseWeeksExpandHeight: 20 + Math.ceil(weekCount / 3) * 108,courseLocation: "",courseTeacher: "",savingCourse: false,editingCourseId: "",editingCourseDate: "",courseWeekdays: [wd(asDate(this.data.selectedDate))],courseWeekdayOptions: DAY_LABELS.map((label, index) => ({ weekday: index + 1, label, selected: index + 1 === wd(asDate(this.data.selectedDate)) })),coursePeriods: [],courseWeeks: [defaultWeek],coursePeriodOptions: periods.map((period) => ({period: period.period,
time: `${period.startTime}–${period.endTime}`,
selected:false,
})),courseWeekOptions: Array.from({ length: weekCount }, (_, index) => {const week = index + 1;const dates = weekDateKeys(table, week);const start = dates[0]?.slice(5).replace("-", "/");const end = dates[6]?.slice(5).replace("-", "/");return {week,
...
return void wx.showToast({ title: "请选择周次", icon: "none" });if (!table)
...
});this.setTabBarHidden(false);if (moving || this._motion?.active.value) {savedPlan={ date:this.data.startDate,id:savedPlanId };pd=this.data.startDate;dirty=true;return;
}
...
},deletePlan() {const editingPlanId = this.data.editingPlanId;if (!editingPlanId) return;const lease = takeLease();if (!lease || lease.account !== acct) return;wx.showModal({title: "删除日程",content: "确定删除这个日程？",confirmText: "删除",confirmColor: "#c0452d",success: (result) => {if (!result.confirm || !leaseCurrent(lease)) return;const plans = sch(acct).plans.filter(
