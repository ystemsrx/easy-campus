...
export function drawAboutPrint(
  ...
  context.fillText(print.wall.today.replace(/-/g, "."), 749, 867);
  context.textAlign = "left";
  context.font = "16px sans-serif";
  print.wall.months.forEach((month, column) => {
    context.fillText(month.label, 178 + column * 44, 917);
  });
  print.wall.rows.forEach((row, rowIndex) => {
    context.fillStyle = "#74796a";
    context.fillText(row.label, 146, 954 + rowIndex * 44);
  ...
  COLORS.forEach((color, index) =>
    square(context, 629 + index * 26, 1260, 18, 5, color),
  );
  context.fillStyle = "#74796a";
  context.font = "17px sans-serif";
  context.fillText("校园很大，日子慢慢来。", 146, 1310);
}
...
export function exportAboutPrint(print: AboutPrint): Promise<string> {
  ...
  drawAboutPrint(canvas.getContext("2d"), print);
